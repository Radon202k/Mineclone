...
face_is_exposed(ChunkVoxels *chunk, s32 x, s32 y, s32 z, FaceOrientation face) {
    ...
    switch (face) {
        case Face_top:    voxelType = voxel_get(chunk->voxels, x, y+1, z); break;
        case Face_bottom: voxelType = voxel_get(chunk->voxels, x, y-1, z); break;
        case Face_left:   voxelType = voxel_get(chunk->voxels, x-1, y, z); break;
        case Face_right:  voxelType = voxel_get(chunk->voxels, x+1, y, z); break;
        case Face_front:  voxelType = voxel_get(chunk->voxels, x, y, z+1); break;
        case Face_back:   voxelType = voxel_get(chunk->voxels, x, y, z-1); break;
...
generate_chunk_mesh_face(ChunkMesh *mesh, s32 x, s32 y, s32 z, 
    ...
    switch (face) {
        case Face_top: {
            mesh->vertices[mesh->vertexIndex++] = (VoxelVertex){{x*d-s,y*d+s,z*d-s}, {0,0}};
            mesh->vertices[mesh->vertexIndex++] = (VoxelVertex){{x*d+s,y*d+s,z*d-s}, {1,0}};
            mesh->vertices[mesh->vertexIndex++] = (VoxelVertex){{x*d+s,y*d+s,z*d+s}, {1,1}};
            mesh->vertices[mesh->vertexIndex++] = (VoxelVertex){{x*d-s,y*d+s,z*d+s}, {0,1}};
        } break;
