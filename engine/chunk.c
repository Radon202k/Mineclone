...
voxel_get(ChunkVoxels *chunk, s32 x, s32 y, s32 z) {
    if (x == -1) {
        ...
        if (leftChunk) {
            platform_debug_print("Tested left chunk\n");
        ...
        ChunkVoxels *rightChunk = 
            chunk_voxels_htable_find(chunk->x+1,chunk->y,chunk->z);
        if (rightChunk) {
            platform_debug_print("Tested right chunk\n");
        ...
        if (bottomChunk) {
            platform_debug_print("Tested bottom chunk\n");
        ...
        ChunkVoxels *topChunk = 
            chunk_voxels_htable_find(chunk->x,chunk->y+1,chunk->z);
        if (topChunk) {
            platform_debug_print("Tested top chunk\n");
        ...
        if (backChunk) {
            platform_debug_print("Tested back chunk\n");
        ...
        ChunkVoxels *frontChunk = 
            chunk_voxels_htable_find(chunk->x,chunk->y,chunk->z+1);
        if (frontChunk) {
            platform_debug_print("Tested front chunk\n");
